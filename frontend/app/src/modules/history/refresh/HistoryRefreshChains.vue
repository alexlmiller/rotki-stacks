...
const { bitcoinChainsData, evmLikeChainsData, solanaChainsData, stacksChainsData, txEvmChains } = useSupportedChains();
...
const refreshChains = computed<ChainData[]>(() => [
  ...
  ...get(stacksChainsData).map(item => ({
    chain: item.id,
    id: item.id,
    name: item.name,
    type: TransactionChainType.STACKS,
  })),
]);
