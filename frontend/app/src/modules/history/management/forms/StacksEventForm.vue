...
import type { NewStacksEventPayload, StacksEvent } from '@/modules/history/events/schemas';
import { HistoryEventEntryType, Zero } from '@rotki/common';
import useVuelidate from '@vuelidate/core';
import dayjs from 'dayjs';
import { isEmpty } from 'es-toolkit/compat';
import { STACKS_CHAIN } from '@/modules/assets/types';
...
interface HistoryEventFormProps {
  data: StandaloneEventData<StacksEvent>;
}
...
const location = ref<string>(STACKS_CHAIN);
...
const rules = {
  address: commonRules.createValidStacksAddressRule(),
  ...
  txRef: commonRules.createValidStacksTxIdRule(),
};
...
function applyEditableData(entry: StacksEvent) {
...
function applyGroupHeaderData(entry: StacksEvent) {
  set(sequenceIndex, get(data)?.nextSequenceId || '0');
...
async function save(): Promise<boolean> {
  ...
  const payload: NewStacksEventPayload = {
...
<template>
  <div>
    ...
    <RuiAccordions>
      <RuiAccordion
        data-cy="stacks-event-form__advance"
        header-class="py-4"
        eager
      >
        <template #header>
