...
import type { StacksEvent } from '@/modules/history/events/schemas';
...
describe('forms/StacksEventForm.vue', () => {
  let wrapper: VueWrapper<InstanceType<typeof StacksEventForm>>;
  ...
  const group: StacksEvent = {
    address: 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR',
    amount: bigNumberify(100),
    asset: asset.symbol,
    counterparty: null,
    entryType: HistoryEventEntryType.STACKS_EVENT,
    eventSubtype: '',
    eventType: 'receive',
    groupIdentifier: '0x2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c',
    identifier: 14344,
    location: 'stacks',
    locationLabel: 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE',
    sequenceIndex: 2411,
    timestamp: 1686495083,
    txRef: '0x1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b',
    userNotes: 'Receive 100 STX from SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE',
  };
  ...
  const createWrapper = (options: ComponentMountingOptions<typeof StacksEventForm> = {
    props: {
      data: { nextSequenceId: '0', type: 'add' },
    },
  }): VueWrapper<InstanceType<typeof StacksEventForm>> =>
    mount(StacksEventForm, {
      global: {
        plugins: [pinia],
        stubs: {
          I18nT: true,
  ...
  it('should show the default state when opening the form without any data', async () => {
    ...
    expect(locationInput.element.value).toBe('stacks');
  ...
  it('should add a new stacks event when form is submitted', async () => {
    wrapper = createWrapper();
    await nextTick();
    await vi.advanceTimersToNextTimerAsync();
  ...
  it('should edit an existing stacks event when form is submitted', async () => {
    wrapper = createWrapper({
      props: {
        data: {
          event: group,
  ...
  describe('actualGroupIdentifier', () => {
    const eventWithActualGroupIdentifier: StacksEvent = {
      ...group,
    ...
    it('should use actualGroupIdentifier when present and disable the field', async () => {
      ...
      await wrapper.find('[data-cy=stacks-event-form__advance] [data-accordion-trigger]').trigger('click');
      await vi.advanceTimersToNextTimerAsync();
